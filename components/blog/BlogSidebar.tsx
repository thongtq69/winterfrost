...
const BlogSidebar = () => (
  <div className="space-y-4">
    <Card className="rounded-[24px] p-5">
      <h3 className="text-lg font-bold text-ink">{siteConfig.brand.name}</h3>
    ...
    <Card className="rounded-[24px] p-5">
      ...
      <Button fullWidth className="mt-3" asChild>
        <a href={siteConfig.links.zalo}>{siteConfig.cta.zaloText}</a>
      </Button>
    </Card>
    <Card className="rounded-[24px] p-5">
      <h4 className="text-base font-semibold text-ink">Kết nối</h4>
      <div className="mt-3 flex gap-3">
        {[{ icon: Facebook, label: 'Facebook', href: siteConfig.links.facebook }, { icon: MessageCircle, label: 'Tiktok', href: siteConfig.links.tiktok }, { icon: Youtube, label: 'YouTube', href: siteConfig.links.youtube }].map(
